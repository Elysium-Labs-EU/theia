...
#
# The version lives in .golangci-lint-version and is read by the Taskfile's
# lint/fix tasks, the pre-commit hook, and any CI workflow that lints.
...
# in play at once (Taskfile v2.11.0, CI workflow v2.11.4, lefthook.yml
...
if grep -nE 'golangci-lint (run|fmt)' Taskfile.yml taskfiles/*.yml >/dev/null 2>&1; then
    fail "Taskfile calls golangci-lint from PATH; use \${GOLANGCI_LINT_VERSION} via go run"
    grep -nE 'golangci-lint (run|fmt)' Taskfile.yml taskfiles/*.yml | sed 's/^/    /' >&2
fi
