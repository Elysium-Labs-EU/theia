...
// piece of the installer that shipped issue #39. It shells out to the real
// script (sourced with THEIA_INSTALL_SOURCE_ONLY=1, which skips running the
...
func runInstallFunc(t *testing.T, script string, env map[string]string) (string, error) {
	...
	cmd.Env = append(os.Environ(), "THEIA_INSTALL_SOURCE_ONLY=1")
	for k, v := range env {
...
func TestPickLatestTagOutOfOrderList(t *testing.T) {
	// Reproduces issue #39: GitHub's /releases list is documented
...
func TestFetchLatestVersionUsesReleasesLatestEndpoint(t *testing.T) {
	...
	out, err := runInstallFunc(t, `fetch_latest_version curl`, map[string]string{"THEIA_API_BASE": srv.URL})
	if err != nil {
...
func TestFetchLatestVersionOutOfOrderListFallback(t *testing.T) {
	...
	// fallback list is out of order the same way as issue #39 — the
